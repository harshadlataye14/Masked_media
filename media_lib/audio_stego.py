...
def Audio_steg(file, n):
    ...
    |               |
/~~\|           /~~\|
\__/            \__/Audio Steganography ENCODING""")
        print("")
        ...
        new_file_name = os.path.join(directory, f"{file_name}_enc_audio{file_extension}")
        with wave.open(new_file_name, 'wb') as fd:
            fd.setparams(song.getparams())
            fd.writeframes(frame_modified)
        print("[INFO] ENCODING DATA Successful")
        print("[INFO] LOCATION:{}".format(os.path.abspath(new_file_name)))
        song.close()
        print("=" * 100)
    ...
    |               |
/~~\|           /~~\|
\__/            \__/Audio Steganography DECODING""")
        print("")
        song = wave.open(file, mode='rb')
        nframes = song.getnframes()
        frames = song.readframes(nframes)
