...
def Text_steg(file, n):
    def EncodeTheText(text):
        ...
        new_filename = os.path.join(directory, f"{os.path.splitext(filename)[0]}_encoded.txt")
        file3 = open(new_filename, 'w+', encoding='utf-8')
        word = []
        for line in file1:
        ...
        print("[INFO] LOCATION:{}".format(new_filename))
        print("=" * 100)
...
| |__| |
|  ()  |
|______|Text Steganography ENCODING""")
        print("")
        count2 = 0
        file1 = open(file, 'r')
        for line in file1:
...
| |__| |
|  ()  |
|______|Text Steganography DECODING""")
